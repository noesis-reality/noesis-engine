...
// MARK: - ModelExporter Extension

extension ModelExporter {
    
    /// Write MoE weights with MXFP4 quantization
    func writeMoEWeightsWithMXFP4(
        ...
        for expertIdx in 0..<config.num_experts {
            // MLP1 (gate and up projections for SwiGLU)
            let mlp1BlocksName = "block.\(blockIdx).mlp.mlp1_weight.blocks"
            let mlp1ScalesName = "block.\(blockIdx).mlp.mlp1_weight.scales"
            
            if let _ = loader.tensorInfo(name: mlp1BlocksName) {
                // Already in MXFP4 format, write directly
                let blocks = try loader.loadTensorData(name: mlp1BlocksName)
                output.write(blocks)
                writePadding(to: output, alignment: 16)
                
                let scales = try loader.loadTensorData(name: mlp1ScalesName)
                output.write(scales)
                writePadding(to: output, alignment: 16)
            } else {
                // Need to quantize from full precision
                let mlp1WeightName = "block.\(blockIdx).mlp.experts.\(expertIdx).gate_up.weight"
                if let _ = loader.tensorInfo(name: mlp1WeightName) {
                    let weights = try loader.loadBFloat16Tensor(name: mlp1WeightName)
                    let (blocks, scales) = MXFP4.quantize(weights: weights)
            ...
            // MLP1 bias
            let mlp1BiasName = "block.\(blockIdx).mlp.mlp1_bias"
            if let _ = loader.tensorInfo(name: mlp1BiasName) {
                let bias = try loader.loadTensorData(name: mlp1BiasName)
                output.write(bias)
                writePadding(to: output, alignment: 16)
            }
            
            // MLP2 (down projection)
            let mlp2BlocksName = "block.\(blockIdx).mlp.mlp2_weight.blocks"
            let mlp2ScalesName = "block.\(blockIdx).mlp.mlp2_weight.scales"
            
            if let _ = loader.tensorInfo(name: mlp2BlocksName) {
                let blocks = try loader.loadTensorData(name: mlp2BlocksName)
                output.write(blocks)
                writePadding(to: output, alignment: 16)
                
                let scales = try loader.loadTensorData(name: mlp2ScalesName)
                output.write(scales)
                writePadding(to: output, alignment: 16)
            } else {
                // Quantize from full precision if needed
                let mlp2WeightName = "block.\(blockIdx).mlp.experts.\(expertIdx).down.weight"
                if let _ = loader.tensorInfo(name: mlp2WeightName) {
                    let weights = try loader.loadBFloat16Tensor(name: mlp2WeightName)
                    let (blocks, scales) = MXFP4.quantize(weights: weights)
            ...
            // MLP2 bias
            let mlp2BiasName = "block.\(blockIdx).mlp.mlp2_bias"
            if let _ = loader.tensorInfo(name: mlp2BiasName) {
                let bias = try loader.loadTensorData(name: mlp2BiasName)
                output.write(bias)
                writePadding(to: output, alignment: 16)
            }
        }
